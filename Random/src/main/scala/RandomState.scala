...
trait RandomState[T] {
  ...
  /**
    ...
    */
  def toStream: Stream[T] = get #:: next.toStream// TODO 12 points
}
...
case class JavaRandomState[T](n: Long, g: Long=>T) extends RandomState[T] {
  def next: RandomState[T] = JavaRandomState(new Random(n).nextLong(),g) // TODO 7 points
  def get: T = g(n) // TODO 5 points
  def map[U](f: (T) => U): RandomState[U] = JavaRandomState(n, f.compose(g))
  //                                                                       Also can use g.andThen(f)
  // TODO 13 points
}
//map[u]?? g(n)哪里定义, identity?
object RandomState {
  ...
  val longToDouble: Long=>Double = {x => x.toDouble/Double.MaxValue} // TODO 4 points
